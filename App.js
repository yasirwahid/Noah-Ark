...
const TT = [
'','','',
'Many people know the story of Noah – how God sent him a vision of the land flooded with water so deep that no one would be able to survive, how he built his ark and saved not only a lot of animals, but also his own wife and his sons and their wives! ',
  ...
  const panResponder = PanResponder.create({
   ...
   // console.log(Object.keys(pan.x));
    ...
    <View  style={{flex:1,backgroundColor:'black'}} /* {...panResponder.panHandlers}*/ >
     ...
     {
       screen == 3 ? (
        <Animated.Text style={[{color:'white',fontSize:22,textAlign:'justify',fontFamily:'Renner'}, {
          transform:[{
            translateX:textAnim
          }]
        }]}>
          <Animated.Text style={{fontWeight:'bold',fontSize:22, textAlign:'justify',
          transform:[{
            translateX:textAnim
          }]
          }}>{TT[screen].split(' ')[0]} </Animated.Text>{TT[screen].split(' ').splice(1).join(' ')}
        </ Animated.Text>
       ) : (
        <Animated.Text style={[{color:'white',fontSize:22,textAlign:'justify',fontFamily:'Renner'}, {
          transform:[{ 
          ...
          <Animated.Text style={{fontWeight:'bold',textAlign:'justify',fontSize:22,transform:[{
            translateX:textAnim
          }]}}>{TT[screen].split(' ')[0]} </Animated.Text>{TT[screen].split(' ').splice(1).join(' ')}
        </Animated.Text>
       )
